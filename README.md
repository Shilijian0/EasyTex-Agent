...
以下为 `demo/demo_showcase.tex` 经 **xelatex 真实编译** 后的 PDF 渲染截图：

### 第 1 页：封面 + 矩阵公式

![第1页-封面与矩阵公式](assets/demo_page_1.png)

> 用户输入 "写一个3x3单位矩阵右乘列向量x，等号右边是全是lambda的向量" → Skill 自动生成带 `equation` 编号、`bmatrix` 环境、`underbrace` 标注的专业公式。

### 第 2 页：协方差矩阵 + 贝叶斯定理

![第2页-协方差与贝叶斯](assets/demo_page_2.png)

> "3x3协方差矩阵，对角线sigma平方，非对角线rho乘sigma" → 自动补全下标、行列对齐、数学粗体 `\boldsymbol{\Sigma}`。

### 第 3 页：排版对照表 + 编译闭环

![第3页-排版对照与编译](assets/demo_page_3.png)

> 展示 Skill 自动修改导言区的前后对照（字体/行距/页边距），以及五步编译验证闭环。

### 第 4 页：致命错误拦截 + 效率对比

![第4页-错误拦截与效率](assets/demo_page_4.png)

> `FONT_NOT_FOUND` 致命信号拦截示例 + 手动 vs Skill 效率对比表。

---

> 💡 完整 `.tex` 源文件见 [`demo/demo_showcase.tex`](demo/demo_showcase.tex)，PDF 见 [`demo/demo_showcase.pdf`](demo/demo_showcase.pdf)。你可以 `git clone` 后在本地 `xelatex demo_showcase.tex` 复现以上效果。
